    ...
    <div class="card shadow-sm">
      ...
      <div class="card-body">
        <form action="function/form_save.php" method="POST" enctype="multipart/form-data">
              ...
              if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                  echo "<option value='" . htmlspecialchars($row["id"]) . "'>" 
                       . htmlspecialchars($row["branch_type"]) ." สาขาที่ ". htmlspecialchars($row["branch_order"]) ." ". htmlspecialchars($row["branch_name"]) 
                       . "</option>";
                }
              }
              ?>
          ...
          <button type="submit" class="btn btn-success w-100">✅ บันทึกการแจ้งซ่อม</button>
        </form>
      </div>
    </div>
  </div>
